...
int stop_send_camera_data_through_udp(struct camera_ctx *ctx) { return 0; }
...
int start_send_camera_data_through_tcp(struct camera_ctx *ctx) {
  ...
  if (listen_tcp_connection(&ctx->camera_tcp_sock_frame, MAX_TCP_CONNECTION) !=
      0) {
  ...
  print(DEBUG, "socket created");
  if (listen_tcp_connection(&ctx->camera_tcp_sock_ack, MAX_TCP_CONNECTION) !=
      0) {
    print(ERROR, "cannot start tcp listen connection");
    return -1;
  }
  ...
  for (;;) {
    ...
    if (send_tcp_message(&ctx->camera_tcp_sock, ctx->data.data,
                         MTU_SIZE_MESSAGE) < 0) {

#else
    if (send_tcp_message(&ctx->camera_tcp_sock_frame, ctx->data.data,
                         ctx->data.size) < 0) {
#endif
      print(ERROR, "cannot send message");
      break;
    ...
    // receive acknowledge
    while (recv_tcp_message(&ctx->camera_tcp_sock_ack, ack_message,
                            ACK_MESSAGE_SIZE)) {
