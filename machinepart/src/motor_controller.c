...
static int
angle_to_delay_microseconds(int angle,
                            int *dest_mcs) {
    assert(dest_mcs);
...
static int set_motor_angle(int motor_pin,
                           int motor_angle) {
    int mcs;
...
// invert if motor pin X
    ...
    angle_to_delay_microseconds(motor_angle,
                                &mcs);
...
int start_configure_motors_angle(struct motors_controller_data *data) {
    ...
    while (data->is_working) {
        set_motor_angle(data->motor_x_gpio_pin,
                        data->motor_x_angle);

        set_motor_angle(data->motor_y_gpio_pin,
                        data->motor_y_angle);
    }
    return 0;
}
